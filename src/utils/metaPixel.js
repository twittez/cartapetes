...
export async function trackMetaEvent(eventName, eventId, customData = {}, hashedUserData = {}) {
  ...
  try {
    ...
    // O envio do lado do servidor (/meta-capi) é o único canal da CAPI.
    // Não fazemos chamada direta ao Graph API pelo navegador: isso exporia o
    // Access Token e seria bloqueado por CORS/ad blockers, gerando dados incorretos.
    console.warn('[Meta CAPI Proxy] Falha ao enviar evento via proxy do servidor.', err);
  }
}
