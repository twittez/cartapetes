...
const UPSELL_ITEMS = [
  {
    id: 'organizador',
    title: 'Organizador de Porta-malas Premium (30L)',
    originalPrice: 89.90,
    price: 49.90,
    discount: '-44% OFF',
    description: 'Fabricado em poliéster oxford 600D reforçado com costuras duplas. Dobrável, com divisórias internas, bolsos de tela laterais e base antiderrapante. Ideal para compras e ferramentas.',
  ...
  {
    id: 'aromatizador',
    title: 'Aromatizador Concentrado Carro Novo (Luxury)',
    originalPrice: 29.90,
    price: 14.90,
    discount: '-50% OFF',
    description: 'Fragrância concentrada importada de alta fixação com notas de couro e camurça premium. Vidro italiano com tampa difusora de madeira nobre. Dura até 45 dias.',
  ...
  {
    id: 'kit_limpeza',
    title: 'Kit Limpeza e Brilho Espelhado Express',
    originalPrice: 39.90,
    price: 19.90,
    discount: '-50% OFF',
    description: 'Acompanha Cera Líquida Premium de Carnaúba (200ml) com proteção UVA/UVB e 1 Toalha de Microfibra profissional ultra macia de 400 GSM (sem costuras).',
...
export default function Upsell({ vehicle, kit, onProceed, onBack }) {
  ...
  const getSelectedItemsArray = () => {
    return UPSELL_ITEMS.filter(item => selectedItems[item.id]);
  };

  const selectedCount = getSelectedItemsArray().length;
  
  const upsellTotal = getSelectedItemsArray().reduce((sum, item) => sum + item.price, 0);
  const totalAmount = kitPrice + upsellTotal;

  const handleFinishUpsell = () => {
    const selectedList = getSelectedItemsArray().map(item => ({
      id: item.id,
      title: item.title,
      price: item.price
    }));
    onProceed(selectedList);
  };

  return (
    <div className="min-h-screen bg-gray-50 text-[#111827] font-sans pb-28 sm:pb-16 antialiased">
      {/* Black Header matching home navbar */}
      <header className="sticky top-0 z-40 bg-black border-b border-gray-900 shadow-md">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 text-[10px] sm:text-xs font-semibold text-gray-400">
            <span className="text-emerald-500">🔒</span> Ambiente Seguro
          </div>
        </div>
      </header>
      ...
      <div className="bg-white border-b border-gray-200 py-3.5 shadow-xs">
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex items-center justify-between text-[11px] sm:text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
            <span className="text-emerald-600 flex items-center gap-1">✓ 1. Veículo</span>
            <span className="text-[#FF5A00] border-b-2 border-[#FF5A00] pb-1">⚡ 2. Personalizar Kit</span>
            <span>3. Pagamento</span>
          </div>
          <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-emerald-500 to-[#FF5A00] w-[66%]"></div>
          </div>
        </div>
      </div>

      {/* Main Container */}
      <main className="max-w-4xl mx-auto px-4 mt-6 lg:mt-10">
        
        {/* Banner Hero */}
        <div className="bg-gradient-to-br from-gray-950 to-gray-900 text-white rounded-2xl p-6 sm:p-8 border border-gray-800 shadow-lg text-center relative overflow-hidden mb-8">
          <div className="absolute top-0 right-0 w-64 h-64 bg-orange-600/10 rounded-full blur-3xl -z-10"></div>
          
          <span className="inline-block bg-[#FF5A00] text-white text-[10px] sm:text-xs font-black px-3 py-1 rounded-full uppercase tracking-wider mb-3.5 animate-pulse">
            Oferta Especial Exclusiva
          </span>
          
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold leading-tight">
            Adicione Acessórios com até <span className="text-[#FF5A00]">50% OFF</span>!
          </h1>
          <p className="text-gray-300 text-xs sm:text-sm max-w-2xl mx-auto mt-2.5 leading-relaxed">
            Aproveite o Frete Grátis já garantido para seu pedido e adicione acessórios essenciais sob medida para o seu <strong className="text-white font-bold">{vehicle}</strong> sem pagar frete adicional.
          </p>

          {/* Short urgency indicator */}
          <div className="mt-4 flex items-center justify-center gap-1.5 text-xs text-orange-400 font-medium">
            <span>⏱️</span> Esta oferta expira assim que você fechar esta página.
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            {UPSELL_ITEMS.map((item) => {
              ...
              return (
                <div 
                  key={item.id}
                  className={`bg-white rounded-2xl border transition duration-300 overflow-hidden shadow-sm flex flex-col sm:flex-row ${
                    isAdded ? 'border-[#FF5A00] ring-1 ring-[#FF5A00]' : 'border-gray-200 hover:border-gray-300'
                  }`}
                >
                  {/* Left Column: Image with Discount Badge */}
                  <div className="w-full sm:w-48 h-48 bg-gray-50 flex-shrink-0 relative border-r border-gray-100">
                    <img 
                      src={item.image} 
                      alt={item.title} 
                      className="w-full h-full object-cover"
                    />
                    <span className="absolute top-3 left-3 bg-[#FF5A00] text-white text-[10px] font-black px-2.5 py-0.5 rounded shadow-sm">
                      {item.discount}
                    </span>
                  </div>
                  ...
                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-[11px] text-gray-500 mb-1.5">
                        <div className="flex items-center text-amber-400 font-serif">★</div>
                        <span className="font-bold text-gray-800">{item.stars}</span>
                        <span>({item.reviews} avaliações)</span>
                      </div>

                      <h3 className="font-extrabold text-gray-900 text-base sm:text-lg leading-snug">
                        {item.title}
                      </h3>
                    ...
                    <div className="mt-5 flex items-center justify-between gap-4 border-t border-gray-50 pt-4">
                      <div>
                      ...
                      <button
                        ...
                        className={`px-5 py-2.5 rounded-xl text-xs font-bold transition duration-200 cursor-pointer ${
                          isAdded 
                            ...
                            : 'bg-[#FF5A00] text-white hover:bg-[#e64f00] shadow-sm'
                        }`}
                      >
                        {isAdded ? 'Remover' : 'Adicionar ao Kit'}
                      </button>
                    </div>
                  </div>
                </div>
          ...
          {/* Right Side: Order Summary Card (Desktop only) */}
          <div className="hidden lg:block">
            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm sticky top-28 space-y-4">
              <h3 className="font-bold text-gray-900 border-b border-gray-100 pb-3 text-xs uppercase tracking-wider">
                Resumo do seu Kit
              </h3>

              {/* Kit detail row */}
              <div className="space-y-1">
                <div className="font-bold text-gray-800 text-xs">{kitName}</div>
                <div className="text-[11px] text-gray-400 font-mono">Veículo: {vehicle}</div>
                <div className="text-right text-xs font-semibold text-gray-800">
                  R$ {kitPrice.toFixed(2).replace('.', ',')}
                </div>
              </div>

              {/* Upsell list row if items are selected */}
              {selectedCount > 0 && (
                <>
                  <hr className="border-gray-100" />
                  <div className="space-y-2.5">
                    <div className="text-[10px] font-bold text-gray-400 uppercase">Acessórios Adicionados:</div>
              ...
              <div className="flex justify-between items-baseline">
                <span className="font-bold text-gray-900 text-xs uppercase">Total</span>
              ...
              <div className="space-y-3 pt-2">
                <button
                  onClick={handleFinishUpsell}
                  className="w-full bg-[#FF5A00] hover:bg-[#e64f00] text-white font-bold py-3.5 rounded-xl text-xs uppercase tracking-wider shadow-md hover:shadow-lg transition duration-200 cursor-pointer text-center"
                >
                  Ir para o Pagamento
                </button>
                ...
                <button
                  onClick={handleFinishUpsell}
                  className="w-full text-center text-xs text-gray-400 hover:text-gray-600 underline font-medium block transition cursor-pointer"
                >
                  Pular acessórios e continuar
                </button>
              </div>

              {/* Secure payment seal */}
              <div className="bg-[#FFF5F2] rounded-xl p-3 text-[10px] text-gray-500 flex items-center gap-2.5 border border-[#FFE2DA]">
                <svg viewBox="0 0 264.00031 375.41" className="h-6 w-auto flex-shrink-0" xmlns="http://www.w3.org/2000/svg">
                ...
                <span className="leading-relaxed">
                  Ambiente seguro processado pela <b>Shopee</b>. Seus dados estão 100% protegidos com criptografia.
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 p-4 flex flex-col sm:hidden shadow-[0_-4px_16px_rgba(0,0,0,0.08)]">
        <div className="flex items-center justify-between mb-3 text-xs">
          <div>
            <span className="text-gray-400 font-medium text-[10px] block">
              {selectedCount === 0 ? 'Sem adicionais' : `+ ${selectedCount} ${selectedCount === 1 ? 'adicional' : 'adicionais'}`}
            </span>
            <span className="text-gray-500 font-semibold">{kitName}</span>
          </div>
          <div className="text-right">
            <span className="text-[10px] text-gray-400 uppercase font-bold block">Total</span>
            <span className="text-xl font-black text-[#FF5A00] leading-none">
              R$ {totalAmount.toFixed(2).replace('.', ',')}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={handleFinishUpsell}
            className="flex-1 bg-[#FF5A00] text-white font-bold py-3.5 rounded-xl text-xs text-center uppercase tracking-wide shadow-sm hover:bg-[#e64f00] transition cursor-pointer"
          >
            Ir para o Pagamento
          </button>
          ...
          <button
            onClick={handleFinishUpsell}
            className="text-[11px] text-gray-400 hover:text-gray-600 underline font-semibold px-2 cursor-pointer"
          >
            Pular
          </button>
        </div>
